...
svmfit <- svm(factor(class)~., data = seismic, cost = .1, kernel = "linear", subset = train)
tune.out <- tune(svm, class~., data = seismic[train,], kernel = "linear",
...
svmfit.opt <- svm(class~., data = seismic[train,], kernel = "radial",
                  gamma = 2, cost = 1, decision.values = T)
fitted <- attributes(predict(svmfit.opt, seismic[train,],
                             decision.values = T))$decision.values

par(mfrow = c(1,2))
rocplot(fitted, seismic[train,"class"], main = "Training data")

# Increasing gamma gives a move "flexible" fit
svmfit.flex <- svm(class~., data = seismic[train,], kernel = "radial",
                  gamma = 50, cost = 1, decision.values = T)
fitted.flex <- attributes(predict(svmfit.flex, seismic[train,],
                             decision.values = T))$decision.values
rocplot(fitted.flex, seismic[train,"class"], add=T, col="cyan")

# Plot onto test data
fitted.test1 <- attributes(predict(svmfit.opt, seismic[-train,],
                             decision.values = T))$decision.values
rocplot(fitted.test1, seismic[-train,"class"], main = "Test data")

fitted.test2 <- attributes(predict(svmfit.flex, seismic[-train,],
                                   decision.values = T))$decision.values
rocplot(fitted.test1, seismic[-train,"class"], add = T, col = "cyan")
```
