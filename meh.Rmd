...
## Roc Curve for Training
plot(perf_train,colorize=TRUE, main="Train-ROC-Step")
abline(a=0, b= 1)
perf_train <- performance(pred,"tpr","fpr",measure="auc")
auc <- round(as.numeric(perf_train@y.values),5)
text(0.75, 0.25, auc, cex = .8)
...
#Roc Curve for Test
plot(perf_test,colorize=TRUE, main="Test-ROC-Step")
abline(a=0, b= 1)
perf_test <- performance(pred,"tpr","fpr",measure="auc")
auc <- round(as.numeric(perf_test@y.values),5)
text(0.75, 0.25, auc, cex = .8)
...
## Quadratic Discriminant Analysis - Step and Lasso
...
## Roc Curve for Training
plot(perf_train,colorize=TRUE, main="Train-ROC-Lasso")
abline(a=0, b= 1)
perf_train <- performance(pred,"tpr","fpr",measure="auc")
auc <- round(as.numeric(perf_train@y.values),5)
text(0.75, 0.25, auc, cex = .8)
...
#Roc Curve for Test
plot(perf_test,colorize=TRUE, main="Test-ROC-Lasso")
abline(a=0, b= 1)
perf_test <- performance(pred,"tpr","fpr",measure="auc")
auc <- round(as.numeric(perf_test@y.values),5)
text(0.75, 0.25, auc, cex = .8)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, fig.height=2, cache=T}
# All system times for Full, Step and Lasso
qda.times <- matrix(c("NA",time1,time2,"NA",rate1.train,rate3.train,"NA",rate2.test,rate4.test),nrow=3,ncol=3, byrow=T)
...
## Regularized Discriminant Analysis -Full and Step Model
...
rocplot(fitted.test1, seismic[-train,"class"], main = "Test data", print.auc=T)
