...
We also did boosting on model 1 (from stepwise variable selection), but the original model performs best, when evaluating by misclassification rate (shown in the table below). Therefore, in the interest of space, we only report the original model.
...
Here we performed random forest classifcations on resulting model from stepwise variable slection. 
Stepwise Model =  genergy + gpuls + nbumps + nbumps2 + nbumps4
Missclassificaion rates for stepwise model's training and test datasets are 6.9% and 5.4%, respectively. This time the ROC curves revealed slightly improved test AUC. 
According to the variable importance plot, most important varibales for predicting next seismic events seem to be nbumps and nbumps4. We can see from ROC curves, that train AUC is still slightly higher than test AUC. 
...
#### Time elasped on Full Model: 5.09  
#### Time elasped on Stepwise Model: 1.90
...
|Stepwise model (Train)| 6.9%|0.997 | nbumps and nbumps4|
|Stepwise model (Test)| 5.4%| 0.776|nbumps and nbumps4 |
...
$$\text{and } \epsilon_i\geq0, \sum_{i=1}^n\epsilon_i\leq C$$

where $C$ is $cost$, a non-negative tuning parameter, and $\epsilon_1,\ldots,\epsilon_n$ are slack variables, that determine how far an observation is allowed to be observed on the wrong side of the decision boundary.

The support vector classifier can alternatively be represented as

$$f(x) = \beta_0 + \sum_{i=1}^n \alpha_0 \langle x,x_i \rangle,$$

where $$\langle x, x_i \rangle = \sum_{j=1}^p x_{\cdot j}x_{ij}.$$
...
```{r, echo = F, message = F, warning = F, fig.height = 8, fig.width=11}
library(ROCR)
par(mfrow = c(2,3))
