...
## 2.1 Logistic Regression-Full Model  Logistic Regression - Step Model
...
log.reg <- matrix(c(time1,time2,time3,rate1.train,rate3.train,rate5.train,rate2.test,rate4.test,rate6.test),nrow=3,ncol=3, byrow=T)
colnames(log.reg) <- c("Full", "Step", "Lasso")
...
```{r, echo = F, message = F, comment = NA, cache=T, fig.height=2.5}
###########
# Model 2=seismic+shift+gpuls+nbumps
# LASSO Variable Selection
###########
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, fig.height=2, cache=T}
# All system times for Full, Step and Lasso
lda.times <- matrix(c(time1,time2,time3,rate1.train,rate3.train,rate5.train,rate2.test,rate4.test,rate6.test),nrow=3,ncol=3, byrow=T)
colnames(lda.times) <- c("Full", "Step", "Lasso")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, fig.height=2, cache=T}
# All system times for Full, Step and Lasso
qda.times <- matrix(c("NA",time2,time3,"NA",rate3.train,rate5.train,"NA",rate4.test,rate6.test),nrow=3,ncol=3, byrow=T)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, fig.height=2, cache=T}
# All system times for Full, Step and Lasso
rda.times <- matrix(c(time1,time2,time3,rate1.train,rate3.train,rate5.train,rate2.test,rate4.test,rate6.test),nrow=3,ncol=3, byrow=T)
colnames(rda.times) <- c("Full", "Step", "Lasso")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, eval=T, cache=T}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, comment=NA, eval=F, cache=T}
#-------------------------------------- 
## Model 2 = seismic+shift+gpuls+nbumps
#--------------------------------------
...
rocplot <- function(pred, truth, add = F,...){
...
```{r, echo = F, message = F, warning = F, fig.height = 4, fig.width=11, fig.cap = "ROC curves for SVM, where training is in blue, test in brown."}
library(ROCR)
par(mfrow = c(1,3))
rocplot(fitted1, seismic[train,"class"], main = "ROC for Linear Kernel", col = "dark cyan")
rocplot(fitted.test1, seismic[-train,"class"], add = T, col = "coral3")

rocplot(fitted2, seismic[train,"class"], main = "ROC for Radial Kernel", col = "dark cyan")
rocplot(fitted.test2, seismic[-train,"class"], add = T, col = "coral3")

rocplot(fitted4, seismic[train,"class"], main = "ROC for Polynomial Kernel", col = "dark cyan")
rocplot(fitted.test4, seismic[-train,"class"], add = T, col = "coral3")
