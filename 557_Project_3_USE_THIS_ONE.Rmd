...
bestmtry <- tuneRF(seismic.train[-16], seismic.train$class, ntreeTry=100, 
     stepFactor=1.5,improve=0.01, plot=TRUE, dobest=FALSE)
```
...
```
We see from the Variable importance plot, that the most important variables are nbumps2, nbumps3, genergy, nbumps4, nbumps, maxenergy, gdenergy, gpuls, and energy. Variables like shift, ghazard, nbumps5 and seismoacoustic are of less important for predicting seismic events.  
...
test.accuracy
```
According to the variable importance plot, most important varibales for predicting next seismic events seem to be nbumps and nbumps4.  
...
|:-------------|----------------:|-----:|------------------------------------:|
|Full Model (Train)| 9.4%| 0.996|nbumps2, nbumps3, genergy, nbumps4, nbumps, maxenergy, gdenergy, gpuls, and energy |
|Full Model (Test)| 5.7%| 0.757| nbumps2, nbumps3, genergy, nbumps4, nbumps, maxenergy, gdenergy, gpuls, and energy|
...
```{r, echo = F, message = F, warning = F, fig.height = 2, fig.cap = "ROC curves for SVM, where training is in blue, test in brown."}
library(ROCR)
par(mfrow = c(1,4))
rocplot(fitted1, seismic[train,"class"], main = "ROC for Linear Kernel", col = "dark cyan")
rocplot(fitted.test1, seismic[-train,"class"], col = "coral3")

rocplot(fitted2, seismic[train,"class"], main = "ROC for Radial Kernel", col = "dark cyan")
rocplot(fitted.test2, seismic[-train,"class"], col = "coral3")

rocplot(fitted4, seismic[train,"class"], main = "ROC for Polynomial Kernel", col = "dark cyan")
rocplot(fitted.test4, seismic[-train,"class"], col = "coral3")
